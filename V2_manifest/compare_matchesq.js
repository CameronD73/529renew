...
tr_el.innerHTML="Triangulate into 529andYou";
tr_el.id="c529";
...
function finishComparisons() {
	...
	if( pendingComparisons > 0 || qQueue.length > 0 ) {
		if ( pendingMismatches > 0 ) {
			...
			document.getElementById("c529").innerHTML="Triangulation into 529andYou failed";
			return;
		}
		alert( `program failure - called finalise with ${pendingComparisons} pending and Q ${qQueue.length}`);
		document.getElementById("c529").innerHTML=`Triangulation into 529andYou ${pendingComparisons} pending`;
		pendingMismatches++;
	...
	*/
	...
	if( !pendingPages ) {
		if(failedInSomeWay){
			document.getElementById("c529").innerHTML="Triangulate into 529andYou (failed)";
			failedInSomeWay=false;
		}
		else{
			document.getElementById("c529").innerHTML="Triangulation into 529andYou Completed";
		}
	}
	else{
	...
	for(let i=0; i<row_container.children.length; i++){
		if(row_container.children[i].hasAttribute("class")){
			...
			if(!ranPrimaryComparison){
				...
				pendingComparisons++;
				document.getElementById("c529").innerHTML="Submitting Comparisons...";
...
tr_el.onclick=function(evt){
	...
	if(tr_el.innerHTML=="Triangulation into 529andYou Completed") return;
...
b529.onclick=function(){
	...
	chrome.extension.sendRequest({url:chrome.extension.getURL('results_tab.html')+query}, function(response) {});
};
