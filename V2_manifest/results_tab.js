...
function createButton(transaction, results){
...
function createCSVButton(transaction, results){
	let newButton=document.createElement('button');
...
function createGEXFButton(transaction, results){
	let newButton=document.createElement('button');
...
function createSVGButton(transaction, results){
	let newButton=document.createElement('button');
...
function createClearFilterButton(){
	...
	newButton.addEventListener('click', function(){
		document.getElementById("selectNameFilter").value="";
  		...
  		chrome.extension.getBackgroundPage().getMatchesFromDatabase(createNameSelector);
  		});
...
function setDisplayModeSelector(){
	...
	widget.value=chrome.extension.getBackgroundPage().getDisplayMode();
	widget.onchange=function(){
		chrome.extension.getBackgroundPage().setDisplayMode(widget.value);
...
function setTextSizeSelector(){
	function setTextSize(){
		let desiredSize=chrome.extension.getBackgroundPage().getTextSize();
		let smallCSS=null;
	...
	widget.value=chrome.extension.getBackgroundPage().getTextSize();
	widget.onchange=function(){
		chrome.extension.getBackgroundPage().setTextSize(widget.value);
		setTextSize();
	};
	setTextSize();
}
function setBuildSelector(){
	...
	widget.value=chrome.extension.getBackgroundPage().getBuild();
	widget.onchange=function(){
		chrome.extension.getBackgroundPage().setBuild(widget.value);
		if(document.getElementById("table_div").hasChildNodes()){
			requestSelectFromDatabase(false);
		}
	};
...
function setAlwaysShowPhaseCheckBox(){
	...
	widget.checked=chrome.extension.getBackgroundPage().getAlwaysShowPhase();
	widget.onclick=function(){
		chrome.extension.getBackgroundPage().setAlwaysShowPhase(widget.checked);
	};
}
function setAlwaysShowLabelsCheckBox(){
	var widget=document.getElementById("alwaysShowLabelsCheckBox");
	widget.checked=chrome.extension.getBackgroundPage().getAlwaysShowLabels();
	widget.onclick=function(){
		chrome.extension.getBackgroundPage().setAlwaysShowLabels(widget.checked);
	};
}
function setAlwaysShowCommonAncestorsCheckBox(){
	var widget=document.getElementById("alwaysShowCommonAncestorsCheckBox");
	widget.checked=chrome.extension.getBackgroundPage().getAlwaysShowCommonAncestors();
	widget.onclick=function(){
		chrome.extension.getBackgroundPage().setAlwaysShowCommonAncestors(widget.checked);
	};
}
function setOmitAliasesCheckBox(){
	var widget=document.getElementById("omitAliasesCheckBox");
	widget.checked=chrome.extension.getBackgroundPage().getOmitAliases();
	widget.onclick=function(){
		chrome.extension.getBackgroundPage().setOmitAliases(widget.checked);
	};
}
function setHideCloseMatchesCheckBox(){
	var widget=document.getElementById("hideCloseMatchesCheckBox");
	widget.checked=chrome.extension.getBackgroundPage().getHideCloseMatches();
	widget.onclick=function(){
		chrome.extension.getBackgroundPage().setHideCloseMatches(widget.checked);
	};
}
...
function colorizeButton(button, cid1_1, cid1_2, cid2_1, cid2_2){
	return function(transaction, results){
		...
		var sharingNamesAndIds=chrome.extension.getBackgroundPage().getSharingNamesAndIds();
...
function createTable12(transaction, results, colorize){
	...
	var build=chrome.extension.getBackgroundPage().getBuild();
	var omitAliases=chrome.extension.getBackgroundPage().getOmitAliases();
	var hideCloseMatches=chrome.extension.getBackgroundPage().getHideCloseMatches();
	...
	for(let i=0; i<results.rows.length; i++) {	//results.rows.length
		...
		if(expectedIds[0]==row.id2_1 && expectedIds[1]==row.id2_2){
			...
			if(showURL && row.id2_1 && row.id2_2){
				...
				//link.setAttribute("href", chrome.extension.getURL('results_tab.html')+"?id=" + numbers2id([row.id2_1, row.id2_2]));
			...
			if(showURL && row.id1_1 && row.id1_2){
				...
				let link = document.createElement("a");
				link.setAttribute("href", chrome.extension.getURL('results_tab.html')+"?id=" + numbers2id([row.id1_1, row.id1_2]));
				link.setAttribute("target", "_blank");
		...
		if(showPhaseHints){
			...
			if(build==37 && colorize){
				if(showPhaseInfo || alwaysShowPhase || alwaysShowLabels)
					chrome.extension.getBackgroundPage().selectSegmentMatchesAndPhaseFromDatabase(colorizeButton(button, row.id1_1, row.id1_2, row.id2_1, row.id2_2), row.ROWID);
				else
					chrome.extension.getBackgroundPage().selectSegmentMatchesFromDatabase(colorizeButton(button, row.id1_1, row.id1_2, row.id2_1, row.id2_2), row.ROWID);
			}
...
function createCSV(transaction, results){
	createCSV12(transaction, results, false, false);
}
	...
	if(matchingSegments.length>0){
		...
		for(let k=0; k<matchingSegmentsArray.length; k++){
			for(let i=0; i<matchingSegmentsArray[k].length; i++) {	//results.rows.length
				...
				{
					let link = document.createElement("a");
					link.setAttribute("href", chrome.extension.getURL('results_tab.html')+"?id=" + numbers2id([matchingSegmentsArray[k][i][5], matchingSegmentsArray[k][i][6]]));
					link.setAttribute("target", "_blank");
				...
				if(build!=37){
					...
					if(sharingNamesAndIds){
						for(let ii=0; ii<sharingNamesAndIds.length; ii++){
							if(sharingNamesAndIds[ii][1]===numbers2id([matchingSegmentsArray[k][i][3], matchingSegmentsArray[k][i][4]])){
								firstIsSharing=true;
								break;
							}
						}
						if(firstIsSharing){
							for(let ii=0; ii<sharingNamesAndIds.length; ii++){
								if(sharingNamesAndIds[ii][1]===numbers2id([matchingSegmentsArray[k][i][5], matchingSegmentsArray[k][i][6]])){
									secondIsSharing=true;
								}
							}
						}
					}
					// Deleted code for button for upgrading from Build 36
					{
						while(tablerow.cells.length<8){
...
function providePhaseClues(rowid, name1, name2, id1_1, id1_2, id2_1, id2_2){
	...
	if(document.getElementById("displayMode").value>2 || document.getElementById("alwaysShowPhaseCheckBox").checked || document.getElementById("alwaysShowLabelsCheckBox").checked){
		chrome.extension.getBackgroundPage().selectSegmentMatchesAndPhaseFromDatabase(createSegmentTable, rowid);
	}
	else{
		chrome.extension.getBackgroundPage().selectSegmentMatchesFromDatabase(createSegmentTable, rowid);
	}
}
function requestSelectFromDatabase(shiftIsDown){
	...
	if(wantPhaseInfo){
		if(shiftIsDown){
			chrome.extension.getBackgroundPage().selectFromDatabaseWithPhaseInfo(createTable2, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
		}
		else{
			chrome.extension.getBackgroundPage().selectFromDatabaseWithPhaseInfo(createTable, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
		}
	}
	else{
		if(shiftIsDown){
			chrome.extension.getBackgroundPage().selectFromDatabase(createTable2, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
		}
		else{
			chrome.extension.getBackgroundPage().selectFromDatabase(createTable, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
		}
	}
}
...
function requestSelectFromDatabaseForCSV(shiftIsDown, altIsDown){
	...
	if(altIsDown){
		chrome.extension.getBackgroundPage().selectFromDatabaseWithNonMatches(createCSV3, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
	}
	else if(shiftIsDown){
		chrome.extension.getBackgroundPage().selectFromDatabaseWithPhaseInfo(createCSV2, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
	}
	else{
		chrome.extension.getBackgroundPage().selectFromDatabase(createCSV, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
	}
}
function requestSelectFromDatabaseForGEXF(){
	...
	chrome.extension.getBackgroundPage().selectFromDatabase(createGEXF, expectedId, document.getElementById("chromosome").options[document.getElementById("chromosome").selectedIndex].value);
}
	...
	var reader = new FileReader();
      ...
      reader.onload = (function(theFile) {
        return function(e) {
        	...
        	if(lines[0]==="Comparison,Chromosome,Start point,End point,Genetic Distance,# SNPs"){
        		let profileNamesAndIds=chrome.extension.getBackgroundPage().getProfileNamesAndIds();
        		if(!profileNamesAndIds){
        			alert("Unable to parse " + theFile.name + ". Please re-enter 529andYou through Family Inheritance: Advanced table view and try again.");
        			return;
        		}
        		let matchingNameIndex=-1;
        		for(let i=0; i<profileNamesAndIds.length; i++){
        			if(lines[1].indexOf(profileNamesAndIds[i][0] + " vs ")==0){
        				matchingNameIndex=i;
        				break;
        			}
        		}
        		if(matchingNameIndex>-1){
        			...
        			chrome.extension.getBackgroundPage().importAggregation(profileNamesAndIds[matchingNameIndex][0], profileNamesAndIds[matchingNameIndex][1], lines, reloadSoon);
        		}
        		else alert("Unable to find profile id for " + lines[1].substr(0, lines[1].indexOf("vs")));
        	}
        	else if(lines[0]==="Name, Match name, Chromosome, Start point, End point, Genetic distance, # SNPs, ID, Match ID"){
        		document.getElementById("docBody").style.cursor="wait";
        		chrome.extension.getBackgroundPage().import529CSV(lines, reloadSoon);
        	}
        		...
        		document.getElementById("docBody").style.cursor="wait";
        		chrome.extension.getBackgroundPage().importWithPhase529CSV(lines, reloadSoon);
        	}
...
function createCommonAncestorChangeListener(rowid){
	return function(){
		...
		if(!badValue){
			chrome.extension.getBackgroundPage().updateCommonAncestors(rowid, commonAncestors);
		}
	};
}
function createPhaseChangeListener(rowid, position){
	return function(evt){
		...
		chrome.extension.getBackgroundPage().updatePhase(rowid, this.selectedIndex, position);
	};
}
function createRelationshipChangeListener(rowid, position){
	return function(){
		...
		if(!badValue){
			chrome.extension.getBackgroundPage().updateRelationship(rowid, relationship, position);
		}
		else{
			this.value=null;
		}
...
function requestDeletionFromDatabase(){
	chrome.extension.getBackgroundPage().deleteAllData(resetAfterDeletion);
}
document.addEventListener('DOMContentLoaded', function () {
	chrome.extension.getBackgroundPage().getMatchesFromDatabase(createNameSelector);
	chrome.extension.getBackgroundPage().getLabelList(createLabelList);
	chrome.extension.getBackgroundPage().echo(createButton);
	chrome.extension.getBackgroundPage().echo(createCSVButton);
	chrome.extension.getBackgroundPage().echo(createGEXFButton);
	chrome.extension.getBackgroundPage().echo(createSVGButton);
	chrome.extension.getBackgroundPage().echo(createImportButton);
	chrome.extension.getBackgroundPage().echo(createClearFilterButton);
	chrome.extension.getBackgroundPage().echo(createDeleteButton);
	chrome.extension.getBackgroundPage().echo(setDisplayModeSelector);
	chrome.extension.getBackgroundPage().echo(setTextSizeSelector);
	chrome.extension.getBackgroundPage().echo(setBuildSelector);
	chrome.extension.getBackgroundPage().echo(setOmitAliasesCheckBox);
	chrome.extension.getBackgroundPage().echo(setHideCloseMatchesCheckBox);
	chrome.extension.getBackgroundPage().echo(setAlwaysShowPhaseCheckBox);
	chrome.extension.getBackgroundPage().echo(setAlwaysShowLabelsCheckBox);
	chrome.extension.getBackgroundPage().echo(setAlwaysShowCommonAncestorsCheckBox);
	...
	document.getElementById("selectNameFilter").onchange=function(){
		if(document.getElementById("selectNameFilter").value.length>0){
			...
			chrome.extension.getBackgroundPage().getFilteredMatchesFromDatabase(document.getElementById("selectNameFilter").value, createNameSelector);
		}
		else{
			document.getElementById("clearFilterButton").style.visibility="hidden";
			chrome.extension.getBackgroundPage().getMatchesFromDatabase(createNameSelector);
